...
.more-box .more:hover {color: #252477; font-weight: 600;}
...
function printGuest(data){//4.
	...
	//let paging = data.paging; / 페이징처리로 안하기 때문에 지움
	...
	if(parseInt(dataCount)===0){//데이터가 없으면
		$(".guest-list-body").empty();//list를 지워라(밑에서 append해줘서)
		return;
	}
	...
	$(".guest-list-body").append(out);//더보기로 할꺼라 html이 아닌 append로 변경
	//$(".page-navigation").html(paging); //페이징처리로 안하기때문에(더보기) 지움
